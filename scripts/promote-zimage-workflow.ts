...
async function promoteZImageWorkflow() {
  console.log("🚀 Starting Z-Image workflow promotion...\n");

  const now = Date.now();

  // 1. 查找最新的工作流包
  console.log("📦 Finding workflow package...");
  const [workflow] = await db
    .select()
    .from(workflowPackageRevisions)
    .where(eq(workflowPackageRevisions.workflowId, "zimage-fast-preview"))
    .limit(1);

  if (!workflow) {
    throw new Error("Workflow package not found. Run seed script first: npm run seed:zimage");
  }
  ...
  // 3. 查找生成配置
  console.log("⚙️  Finding generation profile...");
  const [profile] = await db
    .select()
    .from(generationProfileRevisions)
    .where(eq(generationProfileRevisions.profileKey, "zimage-fast-preview"))
    .limit(1);

  if (!profile) {
    throw new Error("Generation profile not found. Run seed script first: npm run seed:zimage");
  }
  ...
  // 5. 设为全局默认图片生成配置
  console.log("🎯 Setting as default image generation profile...");
  const [existingPointer] = await db
    ...
    .where(
      and(
        ...
        eq(defaultGenerationProfilePointers.capability, "image")
      )
    );
  ...
  console.log(`   Default: global image generation → ${profile.displayName}\n`);
  console.log("🎉 Users can now select this profile in the model picker!\n");
}

promoteZImageWorkflow()
  .then(() => {
